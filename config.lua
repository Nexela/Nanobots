...
function Config.update_settings()
    ...
    Config.poll_rate = settings['global']['nanobots-player-cycle-rate'].value ---@type uint
    Config.entities_per_cycle = settings['global']['nanobots-queued-actions-per-cycle'].value ---@type uint
    Config.actions_per_group = settings['global']['nanobots-queued-actions-per-group'].value ---@type uint
    Config.ticks_between_actions = settings['global']['nanobots-ticks-between-action-groups'].value ---@type uint
    Config.build_tiles = settings['global']['nanobots-build-tiles'].value ---@type boolean
    Config.network_limits = settings['global']['nanobots-network-limits'].value ---@type boolean
    Config.nanobots_auto = settings['global']['nanobots-nanobots-auto'].value ---@type boolean
    Config.equipment_auto = settings['global']['nanobots-equipment-auto'].value ---@type boolean
    Config.afk_time = settings['global']['nanobots-afk-time'].value ---@type uint
    Config.do_proxies = settings['global']['nanobots-fullfill-requests'].value ---@type boolean
    return Config
end
